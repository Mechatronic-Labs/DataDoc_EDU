...
class Line_Graph:
    def __init__(self,df):
        # Get Dataset and separate values and cols
        self.df = df
        self.d = df.to_numpy()
        ...
        # set window title
        self.root.title("Line Graph Creator")
        ...
        self.plot_btn = ttk.Button(self.top_f, text = 'Create One Characteristic Line Graph', command = self.create_line_graph)
        self.plot_btn.pack(side = LEFT, ipady = 5, ipadx = 5)

        self.plot_btn1 = ttk.Button(self.top_f, text = 'Create Two Characteristics Line Graph', command = self.create_2d_line_graph)
        ...
        # Define figure, clear it and set titles      
    ...
    def create_line_graph(self):
        ...
        # Set colors and markers options
    ...
    def create_2d_line_graph(self):
        # Set this is as the second options
        self.oldDim = self.dim
        self.dim = 2
        self.selectGraph = True
        ...
        # Create window
        self.master = Tk()
        self.master.title("Create 2 Characteristics Line Plot")
        self.master.geometry('700x200')
        self.master.configure(background='white')
        ...
        Label(self.master, font = ('times new roman', 10), text = "First Line:", bg = '#ffffff').grid(row = 1, column = 0)

        Label(self.master, font = ('times new roman', 10), text = "Select line color:", bg = '#ffffff').grid(row = 2, column = 0)
        ttk.OptionMenu(self.master, self.col, *self.colors.keys()).grid(row = 2, column = 1, ipady = 5, ipadx = 5)
 

        Label(self.master, font = ('times new roman', 10), text = "Select description:", bg = '#ffffff').grid(row = 2, column = 2)
        ttk.OptionMenu(self.master, self.des, *self.descriptions.keys()).grid(row = 2, column = 3, ipady = 5, ipadx = 5)

        Label(self.master, font = ('times new roman', 10), text = "Second Line:", bg = '#ffffff').grid(row = 3, column = 0)
        
        Label(self.master, font = ('times new roman', 10), text = "Select color:", bg = '#ffffff').grid(row = 4, column = 0)
        ttk.OptionMenu(self.master, self.col1, *self.colors.keys()).grid(row = 4, column = 1, ipady = 5, ipadx = 5)

        Label(self.master, font = ('times new roman', 10),text="Select description:", bg = '#ffffff').grid(row = 4, column = 2)
        ttk.OptionMenu(self.master, self.des1, *self.descriptions.keys()).grid(row = 4, column = 3, ipady = 5, ipadx = 5)
    ...
    def show(self):
        ...
        # What to Do in case of the first option      
        if self.dim == 1:      
            val = self.v.get()
            val1 = self.v1.get()
            if val.isdigit():
        ...
        # Clear Canvas and set titles
            ...
            self.f.clf()
            self.ax = self.f.add_subplot(111)
            self.ax.title.set_text("Line Graph")
            self.ax.set_xlabel('')
            self.ax.set_ylabel('')
            self.ax.plot(x,color = self.colors[self.col.get()] , marker = self.descriptions[self.des.get()], label = val)
        ...
        # What to Do in case of the second option      
        else:
            val = self.v.get()
            val1 = self.v1.get()
            if val.isdigit():
        ...
        # Clear Canvas and set titles
            ...
            self.ax.set_ylabel('')
            self.ax.plot(x,color = self.colors[self.col.get()]  , marker = self.descriptions[self.des.get()],  label = val)
            self.ax.plot(y,color = self.colors[self.col1.get()] , marker = self.descriptions[self.des1.get()], label = val1)
    ...
    def st(self):
        # Create window for data Statistics
        self.master = Tk()
