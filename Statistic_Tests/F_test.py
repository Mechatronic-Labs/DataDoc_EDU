...
class f_test:
    ...
    def ftest(self):
        ...
        if val.isdigit():
            ...
            data1 = self.df[val].values

        if val2.isdigit():
            val2 = int(val2)
            data2 = self.df[val2]
        else:
            data2 = self.df[val2].values
        
        #caclulate f value
        self.f_val = np.var(data1, ddof = 1) / np.var(data2, ddof = 1) # because we estimate mean from data 
        
        if self.f_val<0:
            self.f_val = self.f_val *(-1)
        
        fdistribution = stats.f(len(data1) - 1, len(data2) - 1) # build an F-distribution object
        self.p_value = 1 - fdistribution.cdf(self.f_val)

        #Calculate Critical values
        self.cv =  f.ppf(1.0 - 0.05, len(data1)-1, len(data2)-1) # 0.05% Critical Value
        self.cv1 = f.ppf(1.0 - 0.025,len(data1)-1, len(data2)-1) # 0.025% Critical Value
        self.cv2 = f.ppf(1.0 - 0.01, len(data1)-1, len(data2)-1) # 0.01% Critical Value
        ...
        # find y value on T-Distribution for the specific x value and plot it
        self.a.plot(self.f_val, fdistribution.pdf(self.f_val), 'ro',  label='Value')
        self.a.plot(self.cv,    fdistribution.pdf(self.cv),    'ko',  label='0.05% Critical Value')
        self.a.plot(self.cv1,   fdistribution.pdf(self.cv1),   'bo',  label='0.025% Critical Value')
        self.a.plot(self.cv2,   fdistribution.pdf(self.cv2),   'yo',  label='0.01% Critical Value')
